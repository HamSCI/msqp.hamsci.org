...
Not every MSK144 spot is a meteor. Sporadic E, F-layer propagation, tropospheric propagation, and aircraft scatter all carry MSK144 signals into the same PSKReporter stream and the same recordings, and members of the HamSCI Meteor Scatter Working Group who have reviewed MSQP audio report that many spots are of those kinds. Separating meteor scatter from the other modes is one of the research questions, and the classifier exists for that reason. For the platform, a spot records that a path existed; which mechanism carried it is a separate question, and any scoring rule that depends on the answer belongs in the R5 conversation with AF8A.
...
**There is no event-scoped live view.** Two live tools already exist, and the team should know them. Whether the platform integrates with them or stands alone is a design decision for the trade study. [PingJockey Central](https://www.pingjockey.net/cgi-bin/pingtalk) is the chat page where meteor scatter operators arrange contacts and compare notes on shower activity in real time. PSKReporter shows any operator, within minutes, who is hearing them, filtered by callsign, band, and mode. What neither provides is a view of the event itself: which entrants are active, which paths are working on 6 and 10 meters at once, how the running score stands, and how this shower compares with the last one. That is the dashboard R6 describes.

**Zenodo metadata is typed by hand.** Each participant creates a Zenodo account, locates the WSJT-X save directory, zips the audio files (Zenodo caps a deposit at 100 files, so a zip is mandatory), selects the HamSCI community, generates a DOI, and types the deposit metadata to a naming convention: callsign, six-character grid square, dates operated, and the event name. Station details (transmit or receive, radio, antennas, power) are needed alongside it. The platform should collect that station information from each participant once, at registration, and build the Zenodo metadata from it (R7).
...
```
  ...
  50.260 MHz (6 m), 28.145 MHz (10 m)       (ka9q-radio, MSK144 decoder)
       │                                             │
       │ registration, ADIF logs, ALL.TXT, WAV audio │ MSK144 spots
       │                                             ▼
       │                                    ┌──────────────────────┐
       │                                    │  PSKReporter         │
       │                                    │  spot archive        │
...
**Ingest.** Two data streams arrive by different routes. Participant logs (WSJT-X `wsjtx_log.adi` ADIF files and `ALL.TXT`) are uploaded after the event and parsed into per-QSO records. PSKReporter spots are pulled on a schedule for the event window, filtered to the MSQP frequencies and to MSK144, deduplicated, and stored. The raw feed is the engineering challenge here (280 million lines for a single event, of which MSK144 spots are a small fraction), so the ingest filters before it stores.
...
| R4 | **Spot ingest.** Automated, scheduled retrieval of PSKReporter data for each event window, filtered to MSK144 on the MSQP frequencies before storage. The raw feed is large (reference: 280 million lines for the December 2025 event) and MSK144 spots are a small fraction of it, so the requirement is a filter that keeps up with the feed, with the raw and retained volumes documented. |
...
| R6 | **Live event dashboard.** During an event, a near-real-time public view: propagation paths on a map, band and grid activity, entrant leaderboard, and a per-station view an operator can watch while operating. Initial target: spots visible within 15 minutes of being reported. PSKReporter already shows an individual operator who is hearing them within minutes, filtered by callsign, band, and mode; the dashboard adds the event-level view: entrants, both bands, running scores. |
| R7 | **Zenodo deposit integration.** A guided upload flow that builds Zenodo metadata from the entrant's registration and deposits on the participant's behalf, into the participant's own Zenodo account, through Zenodo's OAuth authorization and REST API. The deposit is submitted to the HamSCI community and the returned DOI is recorded against entrant and event. Development and testing run against the Zenodo sandbox; only production writes to the real community. The existing manual path stays documented and available. |
...
**On R1 and R2.** The operating conventions (band-alternation schedule, transmit-slot assignment by antenna heading, receiver AGC off) are set by the HamSCI Meteor Scatter Working Group and are under review; the band-alternation interval in particular may change, because a QSO can take longer than the current slot. Treat every convention as data in the event registry, versioned per event, so that a change is a configuration edit. The AGC instruction is a science requirement the platform should carry into the guidance it gives participants.

**On R4.** The volume figure describes the raw PSKReporter feed, of which MSK144 spots on the MSQP frequencies are a small fraction. The engineering task is to filter that feed reliably and reproducibly and to keep the filtered result queryable; the raw feed does not need to be kept. Measure the real data early in semester 1, record the raw and retained volumes in the requirements specification, and design storage against the retained figure.
...
**On R7.** The deposit is made into the participant's own Zenodo account, with the participant authorizing msqp.hamsci.org once through Zenodo's OAuth flow. Ownership of each record therefore stays with the operator who made the recording, as it does today, and the platform holds no audio it does not need. This is the advisor's decision (2026-09-10); the alternative, a HamSCI service account owning every deposit, was considered and set aside because it would change the authorship of the archive.
...
**The Working Group may add showers.** The HamSCI Meteor Scatter Working Group is considering other 2027 showers. The Orionids (21–22 October 2026) fall too early in the fall semester for a meaningful run, and the Lyrids in late April fall after the workshop. Any event the Working Group adds is a bonus test, and the event registry (R1) should make adding one a configuration change.
...
- **Data engineering:** schema design against measured data, filtering a high-volume feed down to the records that matter, deduplication and reconciliation across two independent sources, and a storage and query strategy sized to what is kept.
...
| **PingJockey Central** | A real-time chat page (pingjockey.net) where meteor scatter operators arrange contacts and compare notes on shower activity. |
| **Ping** | A brief burst of signal, milliseconds to seconds long, scattered from a meteor trail. |
...
12. Suggs, R. M., NN4NT (2026), *Forward Scatter Meteor Radar: The Science Behind the Pings*, tutorial presented at the 2026 HamSCI Workshop, March 2026. Slides: https://www.hamsci.org/sites/default/files/article/Workshop_2026/Meteor_Scatter_Science_Behind_the_Pings_HamSCI_conference_RevB.pdf . Video: https://www.youtube.com/watch?v=8vOfv4Kp7hs
